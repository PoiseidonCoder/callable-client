  ...
  return (
    <div>
      <div className=" fixed top-0 left-0 w-full h-16 bg-background/90 text-foreground z-50 flex justify-between items-center ">
        <Link href={"/"}>
          <Image src={"/images/logo-app.png"} alt="Logo App" width={80} height={80} className="ml-4 object-cover" />
        </Link>
        <Sheet>
        ...
        <div className="flex justify-center items-center gap-4 mr-4">
          <div className="flex flex-col sm:flex-row justify-center items-center sm:gap-4 ">
            <Link className="text-center py-1 min-w-23 bg-btn-gradient rounded-[5px]" href={AUTH_ROUTES.LOGIN}>
              {t("login")}
            </Link>
            <Link className="text-center py-1 min-w-23 border-2 rounded-[5px]" href={AUTH_ROUTES.REGISTER}>
              {t("register")}
            </Link>
          </div>
